...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(oneDigit.format(duration.toMillis()));
        Duration ct = mediaPlayer.getCurrentTime();
        currentText.setText(oneDigit.format(ct.toMillis()));
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        ...
        currentText.setText(oneDigit.format(ms));
    ...
    private void handleSlide(Event event){
         if(mediaPlayer != null) {
            Duration change = new Duration(timeSlider.getValue());
            mediaPlayer.seek(change);
            mediaPlayer.play();
            currentText.setText(oneDigit.format(change.toMillis()));
        }
    }
    @FXML
    private void handleSlide2(Event event){
        if(mediaPlayer != null) {
            mediaPlayer.pause(); 
        } 
    }
